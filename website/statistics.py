...
class Statistics:
    ...
    def get_chart_data(self, start_date: datetime.datetime, end_date:
        ...
        unique_hits = [{"x": date, "y": len(ip_set)} for date, ip_set in
                       unique_hits_dict.items()]

        return hits, unique_hits

    def get_unique_visitors(self, start_date: datetime.datetime, end_date:
                            datetime.datetime) -> int:
        query = (self.db.session.query(
            self.model).group_by(self.model.remote_address))
